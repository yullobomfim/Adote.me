...
#Filtrar os pets por usuário
@login_required
def seus_pets(request):
    if request.method == "GET":
        pet = Pet.objects.filter(usuario=request.user)
        return render(request, 'seus_pets.html', {'pet': pet})

#Remover pets do usuario
@login_required
def remover_pet(request, id):
    pet = Pet.objects.get(id=id)
# Verificar se o pet pertence ao usuario logado para poder deletar o pet    
